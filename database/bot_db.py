...
def sql_create():
    ...
    db.execute('''CREATE TABLE IF NOT EXISTS relation_table 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT,
                    cake VARCHAR(255),
                    taste VARCHAR(255),
                    filling VARCHAR(255),
...
def insert_data():
    ...
    data = [
        ('Эклеры', 'Шоколадный', 'Шоколадный ганаш', 'Сахарная пудра'),
        ('Эклеры', 'Шоколадный', 'Шоколадный ганаш', 'Шоколадный соус'),
        ('Эклеры', 'Сливочный', 'Сливочный крем', 'Сахарная пудра'),
        ('Бисквитные рулеты', 'Шоколадный', 'Клубника', 'Шоколадный соус'),
        ('Бисквитные рулеты', 'Шоколадный', 'Малина', 'Шоколадный соус'),
        ('Бисквитные рулеты', 'Шоколадный', 'Вишня', 'Шоколадный соус'),
        ('Бисквитные рулеты', 'Сливочный', 'Клубника', 'Взбитые сливки'),
        ('Бисквитные рулеты', 'Сливочный', 'Малина', 'Взбитые сливки'),
        ('Бисквитные рулеты', 'Сливочный', 'Вишня', 'Взбитые сливки'),
        ('Брауни', 'Шоколадный', 'Грецкие орехи', 'Глазурь'),
        ('Брауни', 'Шоколадный', 'Миндаль', 'глазурь'),
        ('Брауни', 'Шоколадный', 'Фундук', 'глазурь'),
        ('Пироги', 'Шоколадный', 'Шоколад', 'Сахарная пудра'),
        ('Пироги', 'Сырный', 'Сыр', 'Сахарная пудра'),
        ('Пироги', 'Клубничный', 'Клубника', 'Сахарная пудра'),
        ('Пироги', 'Малиновый', 'Малина', 'Сахарная пудра'),
        ('Пироги', 'Вишневый', 'Вишня', 'Сахарная пудра'),
        ('Муссы', 'Шоколадный', 'Шоколадный мусс', 'Сахарная пудра'),
        ('Муссы', 'Вишневый', 'Вишневый мусс', 'Сахарная пудра'),
        ('Муссы', 'Карамельный', 'Карамельный мусс', 'Сахарная пудра'),
        ('Замороженные торты', 'Сливочный', 'Фруктовое пюре', 'Шоколадный соус'),
        ('Профитроли', 'Сливочный', 'Сливочный крем', 'Сахарная пудра'),
        ('Профитроли', 'Ванильный', 'Мороженое', 'Шоколадный соус'),
    ]

    query = "INSERT INTO relation_table (cake, taste, filling, topping) VALUES (?, ?, ?, ?)"
...
def get_button_names_from_relation_table_by_cake_taste():
    button_names = cursor.execute('''SELECT DISTINCT taste FROM relation_table''').fetchall()
    return button_names


def get_button_names_from_relation_table_by_cake_type():
    button_names = cursor.execute('''SELECT DISTINCT cake FROM relation_table''').fetchall()
    return button_names

def get_sorted_data_from_db(field):
    query = f"SELECT DISTINCT {field} FROM relation_table"
    cursor.execute(query)
    sorted_data = cursor.fetchall()
    return sorted_data

def get_desrription(descr, table_name, key, data):
    # query = f"SELECT {descr} FROM {table_name} WHERE {key}=?, ({data},)"

    cursor.execute(f"SELECT {descr} FROM {table_name} WHERE {key}=?", (f"{data}",))
    desrription = cursor.fetchone()
    return desrription
